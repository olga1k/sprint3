...
// Exercise 1: Get the array of all directors. TESTS PASSED
...
// Exercise 2: Get the films of a certain director TESTS PASSED
function getMoviesFromDirector(array, director) {
  let moviesOneDirector = array.filter((movie) => movie.director === director);
  return moviesOneDirector;
  // console.log("EXERCICE 2 ->", moviesOneDirector);
}

getMoviesFromDirector(movies, 'Francis Ford Coppola');

// Exercise 3: Calculate the average of the films of a given director. PASSED
function moviesAverageOfDirector(array, director) {
  let moviesOneDirector = array.filter((movie) => movie.director === director);
  let averagePoints = moviesOneDirector.reduce((a, b) => ({
    score: a.score + b.score
  }));
  averagePoints = parseFloat(
    (averagePoints.score / moviesOneDirector.length).toFixed(2)
  );

  console.log(
    'EXERCICE 3 ->',
    averagePoints,
    typeof averagePoints,
    moviesOneDirector
  );
...
// Exercise 4:  Alphabetic order by title TESTS PASSED
function orderAlphabetically(array) {
  let titles = array.map((movie) => movie.title);
  //let moviesAToZ = array.sort((a, b) => (a.title < b.title) ? -1 : (b.title < a.title) ? 1 : 0);
  let moviesAToZ = titles.sort((a, b) => (a > b ? 1 : a < b ? -1 : 0));
  let movies20 = moviesAToZ.slice(0, 20);
  //  console.log("EXERCICE 4 ->", movies20);
...
orderByYear(movies);
//console.log('EXERCICE 5 ->', arraySorted);
...
function moviesAverageByCategory(array, genre) {
  let moviesGenre = [];
  let averagePoints = 0;
  let genre2 = genre.charAt(0).toUpperCase() + genre.slice(1).toLowerCase();
  moviesGenre = array.filter((a) => a.genre.some((a) => a === genre2));

  for (let i = 0; i < moviesGenre.length; i++) {
    if (moviesGenre[i].score === undefined) {
      moviesGenre[i].score = 0;
    }
  }
  if (moviesGenre.length > 0) {
    averagePoints = moviesGenre.reduce((a, b) => a + (b.score || 0), 0);

    averagePoints = parseFloat((averagePoints / moviesGenre.length).toFixed(2));
  } else {
    averagePoints = 0;
  }
  console.log('EXERCICE 6 ->', moviesGenre, averagePoints);
  return averagePoints;
}
moviesAverageByCategory(movies, 'animation');

console.log(7);
// Exercise 7: Modify the duration of movies to  //ONLY WITH NEW METHODS? //try to use destructuring


function hoursToMinutes(array) {
  let hour, minutes, hourToMin, moviesToMinutes;
  let hours = [];
  let newArray = [...array];

  newArray = newArray.map((x) => {
    if (!x.duration || typeof x.duration !== 'string') {
      return { duration: '0h 0min' }; 
    }
    return x.duration.split(' ');
  });

  for (let i = 0; i < newArray.length; i++) {
    if (newArray[i][0]) {
      hour = parseInt(newArray[i][0].replace('h', ''));
    } else {
      hour = 0;
    }

    if (newArray[i][1]) {
      minutes = parseInt(newArray[i][1].replace('min', ''));
    } else {
      minutes = 0;
    }

    hourToMin = hour * 60 + minutes;
    hours.push(hourToMin);
  }
  newArray.map((movie, index) => {
    movie.duration = hours[index]; 
  });
  moviesToMinutes = [...newArray]; 

 // console.log('duration array', moviesToMinutes);
...
function bestFilmOfYear(array, year) {
  movieOfYear = array.filter((movie) => movie.year === year);

  if (movieOfYear.length > 0) {
    movieOfYear.sort((a, b) => {
       return b.score - a.score;
    });
    bestMovie = [movieOfYear[0]];
  } else {
    bestMovie = [];
  }
  //console.log('best', bestMovie);
  return bestMovie;
}
bestFilmOfYear(movies, 2017);
